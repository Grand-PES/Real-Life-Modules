...
function m.hook_season()
    if not m.season_addr then
        m.season_addr = hex_to_number(memory.safe_search(
            "\xff\xff\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x04\x00\x00\x00\x11\x00\x00", startAddress,
            endAddress))
    end
...
function m.hook_champion(tid)
    if not m.champion_addr then
        m.champion_addr = memory.safe_search(
            memory.pack("u8", tid) ..
            "\x00" .. memory.pack("u16", m.current_year().dec - 1) .. memory.pack("u8", tid) .. "\x00\x00\x00",
            tablestartAddress, tableendAddress)
    end
...
function m.league_champion(tid)
    if not m.leagues_champions[tid] then
        ...
        log(memory.hex(v2))
        local search = memory.safe_search(v1 .. "\x00" .. m.current_year().hex .. v1 .. "\x00\x00\x00", tablestartAddress,
            tableendAddress)
